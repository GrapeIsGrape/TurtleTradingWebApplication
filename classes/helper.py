...
def xx():
    ...
    if len(schedule['market_open']) > 0:
        local_open = schedule['market_open'].dt.tz_convert(hk_timezone)
        local_close = schedule['market_close'].dt.tz_convert(hk_timezone)
        current_time = datetime.now()
        if local_open <= current_time <= local_close:
            market_is_open = True
